...
exports.classesByLocation = async (req, res) => {
  if (req.body.coordinates) {
    ...
    var disciplineToSearch = req.query.disciplineToSearch
      ? JSON.parse(req.query.disciplineToSearch)
      : "";
  }
  ...
  if (!disciplineToSearch) {
    ...
    const userDisciplines = await User.find(req.profile._id)
      ...
      .select("-_id")
      .exec();
    disciplines = userDisciplines[0].disciplines.map(
      (discipline) => discipline
    );
  } else {
    disciplines = disciplineToSearch.map((discipline) =>
      mongoose.Types.ObjectId(discipline)
    );
  }
...
exports.classesByTeacher = async (req, res) => {
  Classes.find({ teacher: req.teacher._id })
    .populate("discipline")
    .exec((err, classes) => {
      if (err || !classes) {
...
exports.deleteClass = async (req, res) => {
  Classes.findByIdAndDelete(req.class._id).exec((err, result) => {
    if (err) {
      return res.status(400).json(errorHandler(err));
    }
    return res.status(200).json(result);
...
exports.updateClass = async (req, res) => {
  const { hourClassPrice, classId } = req.body;
  console.log(hourClassPrice);
  Classes.findOne({ teacher: req.teacher._id, _id: classId }).exec(
    (err, classe) => {
      classe.hourClassPrice = hourClassPrice;
      classe.save((err, result) => {
        if (err) {
          console.log(err);
          return res.status(400).json(errorHandler(err));
        }

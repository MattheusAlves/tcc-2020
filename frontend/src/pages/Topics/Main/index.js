...
const Topic = ({navigation}) => {
  ...
  async function getUserDisciplines() {
    return api
      ...
      .then((response) => {
        ...
        const formatedUserDisciplines = response.data.disciplines.map(
          (discipline) => {
  ...
  const onCloseChip = useCallback(
    (id) => {
      if (userDisciplines.length > 1) {
        setUserDisciplines([
          ...userDisciplines.filter((item) => item.id != id),
        ]);
      } else {
        setUserDisciplines([]);
        setTopics([]);
  ...
  const addCategory = useCallback(
    (category) => {
      ...
      setUserDisciplines([
        ...userDisciplines,
        {name: category.disciplineName, id: category.id},
      ]);
    },
    [userDisciplines],
  );
  ...
  return error ? (
    <>
      <ConnectionError />
      <Dialog
        dialogState={dialogError}
        onDismiss={() => setDialogError(false)}
        title="Error!"
          ...
          <ScrollView
            ...
            {loading === true ? (
              ...
              <View style={styles.header}>
                <Accordion title="Mostrar disciplinas">
                ...
                <TouchableOpacity style={styles.newTopicWrapper} onPress={() => navigation.navigate('CreateTopic')}> 
                  <Text style={styles.newTopic}>Novo Tópico</Text>
                </TouchableOpacity>
              </View>
            )}
            {topics.length < 1 && loading !== true && !error && (
              <TopicsNotFound navigation={navigation} />
            )}
            {topics.length >= 1 && (
              <Topics
                topics={topics}
